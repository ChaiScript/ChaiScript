...
namespace Catch {
  ...
  Approx::Approx(double value)
      : m_epsilon(static_cast<double>(std::numeric_limits<float>::epsilon()) * 100.), m_margin(0.0), m_scale(0.0), m_value(value) {}
  ...
  namespace {
    ...
    public:
      explicit TestGroup(IEventListenerPtr &&reporter, Config const *config)
          : m_reporter(reporter.get()), m_config{config}, m_context{config, CATCH_MOVE(reporter)} {
...
namespace Catch {
  ...
  TestCaseInfo::TestCaseInfo(StringRef _className,
                             ...
                             SourceLineInfo const &_lineInfo)
      : name(_nameAndTags.name.empty() ? makeDefaultName() : _nameAndTags.name), className(_className), lineInfo(_lineInfo) {
...
namespace Catch {
  ...
  ReporterConfig::ReporterConfig(
      ...
      : m_stream(CATCH_MOVE(_stream)), m_fullConfig(_fullConfig), m_colourMode(colourMode), m_customOptions(CATCH_MOVE(customOptions)) {}
  ...
  SectionStats::SectionStats(SectionInfo &&_sectionInfo,
      ...
      : sectionInfo(CATCH_MOVE(_sectionInfo)), assertions(_assertions), durationInSeconds(_durationInSeconds), missingAssertions(_missingAssertions) {}
  ...
  TestCaseStats::TestCaseStats(TestCaseInfo const &_testInfo,
                               ...
                               std::string &&_stdErr,
                               bool _aborting)
      : testInfo(&_testInfo), totals(_totals), stdOut(CATCH_MOVE(_stdOut)), stdErr(CATCH_MOVE(_stdErr)), aborting(_aborting) {}
    ...
    void Parser::writeToStream(std::ostream &os) const {
      ...
      for (auto &cols : rows) {
        auto row = TextFlow::Column(CATCH_MOVE(cols.left))
                       .width(optWidth)
                       .indent(2)
            + TextFlow::Spacer(4) + TextFlow::Column(static_cast<std::string>(cols.descriptions)).width(consoleWidth - 7 - optWidth);
        os << row << '\n';
      }
    }
  ...
  Clara::Parser makeCommandLineParser(ConfigData &config) {
    ...
    auto const setGeneratorFilter = [&](std::string const &generatorFilter) {
      if (generatorFilter != "*") {
        ...
        if (!parsedIndex) {
          return ParserResult::runtimeError("Could not parse '" + generatorFilter + "' as generator index");
        }
      }
    ...
    auto cli
        ...
        | Opt(accept_many, setGeneratorFilter, "index spec")
            ["-g"]["--generator-index"]("specify generator elements to try")
        | Opt(accept_many, setPathFilter, "path filter spec")
            ["-p"]["--path-filter"]("qualified path filter")
        ...
        | Opt(setTestOrder, "decl|lex|rand")
            ["--order"]("test case order (defaults to rand)")
        ...
        | Opt(config.prematureExitGuardFilePath, "path")
            ["--premature-exit-guard-file"]("create a file before running tests and delete it during clean exit")
...
namespace Catch {
  ...
  ColourImpl::ColourGuard::ColourGuard(ColourGuard &&rhs) noexcept
      : m_colourImpl(rhs.m_colourImpl), m_code(rhs.m_code), m_engaged(rhs.m_engaged) {
    rhs.m_engaged = false;
  }
...
namespace Catch {
  ...
  namespace Detail {
    namespace {
      ...
      public:
        DebugOutStream()
            : m_streamBuf(Detail::make_unique<StreamBufImpl<OutputDebugWriter>>()), m_os(m_streamBuf.get()) {}
...
namespace Catch {
  ...
  JsonObjectWriter::JsonObjectWriter(JsonObjectWriter &&source) noexcept
      : m_os{source.m_os}, m_indent_level{source.m_indent_level}, m_should_comma{source.m_should_comma}, m_active{source.m_active} {
...
namespace Catch {
  ...
  namespace {
    ...
    public:
      StreamRedirect()
          : m_cout(Catch::cout(), m_redirectedOut.get()), m_cerr(Catch::cerr(), m_redirectedErr.get()), m_clog(Catch::clog(), m_redirectedErr.get()) {}
    ...
    public:
      ...
      std::string getContents() {
        ...
        std::rewind(m_file);
        while (current_pos > 0) {
          auto read_characters = std::fread(buffer,
                                            1,
  ...
  RedirectGuard::RedirectGuard(bool activate, OutputRedirect &redirectImpl)
      : m_redirect(&redirectImpl), m_activate(activate), m_previouslyActive(redirectImpl.isActive()) {
  ...
  RedirectGuard::RedirectGuard(RedirectGuard &&rhs) noexcept
      : m_redirect(rhs.m_redirect), m_activate(rhs.m_activate), m_previouslyActive(rhs.m_previouslyActive), m_moved(false) {
    rhs.m_moved = true;
  }
...
namespace Catch {
  ...
  ReporterSpec::ReporterSpec(
      ...
      Optional<ColourMode> colourMode,
      std::map<std::string, std::string> customOptions)
      : m_name(CATCH_MOVE(name)), m_outputFileName(CATCH_MOVE(outputFileName)), m_colourMode(CATCH_MOVE(colourMode)), m_customOptions(CATCH_MOVE(customOptions)) {}
...
namespace Catch {
    ...
    namespace {
      struct GeneratorTracker final : TestCaseTracking::TrackerBase,
        ...
        acquire(TrackerContext &ctx,
          ...
          } else if (ITracker *childTracker = currentTracker.findChild(
                         nameAndLocation)) {
            assert(childTracker);
  ...
  namespace Detail {
    ...
    public:
      ...
      void removeMessage(unsigned int messageId) {
        ...
        //       in debug mode.
        auto iter = std::find_if(messages.begin(),
                                 messages.end(),
  ...
  RunContext::RunContext(IConfig const *_config, IEventListenerPtr &&reporter)
      : m_runInfo(_config->name()), m_config(_config), m_reporter(CATCH_MOVE(reporter)), m_outputRedirect(makeOutputRedirect(m_reporter->getPreferences().shouldRedirectStdOut)), m_abortAfterXFailedAssertions(m_config->abortAfter()), m_reportAssertionStarting(m_reporter->getPreferences().shouldReportAllAssertionStarts), m_includeSuccessfulResults(m_config->includeSuccessfulResults() || m_reporter->getPreferences().shouldReportAllAssertions), m_shouldDebugBreak(m_config->shouldDebugBreak()) {
  ...
  IGeneratorTracker *RunContext::createGeneratorTracker(
    ...
    assert(
        currentTracker.nameAndLocation() != nameAndLoc && "Trying to create tracker for a generator that already has one");
...
namespace Catch {
  ...
  Section::Section(SectionInfo &&info)
      : m_info(CATCH_MOVE(info)), m_sectionIncluded(getResultCapture().sectionStarted(m_info.name, m_info.lineInfo, m_assertions)) {
  ...
  Section::Section(SourceLineInfo const &_lineInfo,
                   StringRef _name,
                   const char *const)
      : m_info({"invalid", static_cast<std::size_t>(-1)}, std::string{}), m_sectionIncluded(getResultCapture().sectionStarted(_name, _lineInfo, m_assertions)) {
...
namespace Catch {
  namespace TestCaseTracking {
    ...
    SectionTracker::SectionTracker(NameAndLocation &&nameAndLocation, TrackerContext &ctx, ITracker *parent)
        : TrackerBase(CATCH_MOVE(nameAndLocation), ctx, parent), m_trimmed_name(trim(StringRef(ITracker::nameAndLocation().name))) {
      if (m_parent) {
        ++m_sectionOnlyDepth;
      }
    }
    ...
    bool SectionTracker::isComplete() const {
      ...
      if (filterIndex < m_filterRef->size()) {
        ...
        if (m_newStyleFilters && (*m_filterRef)[filterIndex].type != PathFilter::For::Section) {
          return true;
        }
...
namespace Catch {
  namespace TextFlow {
    void AnsiSkippingString::preprocessString() {
      for (auto it = m_string.begin(); it != m_string.end();) {
        ...
        if (it != m_string.end()) {
          ...
          while (it != m_string.end() && isUtf8ContinuationByte(*it)) {
            ++it;
          }
        }
      }
    ...
    void AnsiSkippingString::const_iterator::advance() {
      ...
      m_it++;
      // Skip UTF-8 continuation bytes
      while (m_it != m_string->end() && isUtf8ContinuationByte(*m_it)) {
        m_it++;
      }
      tryParseAnsiEscapes();
    }
    ...
    Column::const_iterator::const_iterator(Column const &column)
        : m_column(column), m_lineStart(column.m_string.begin()), m_lineEnd(column.m_string.begin()), m_parsedTo(column.m_string.begin()) {
...
namespace Catch {
  ReporterBase::ReporterBase(ReporterConfig &&config)
      : IEventListener(config.fullConfig()), m_wrapped_stream(CATCH_MOVE(config).takeStream()), m_stream(m_wrapped_stream->stream()), m_colour(makeColourImpl(config.colourMode(), m_wrapped_stream.get())), m_customOptions(config.customOptions()) {}
...
namespace Catch {
  ...
  namespace {
    ...
    public:
      ...
      ConsoleAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats, ColourImpl *colourImpl_, bool _printInfoMessages)
          : stream(_stream), stats(_stats), result(_stats.assertionResult), colour(Colour::None), messages(_stats.infoMessages), colourImpl(colourImpl_), printInfoMessages(_printInfoMessages) {
        switch (result.getResultType()) {
          case ResultWas::Ok:
  ...
  ConsoleReporter::ConsoleReporter(ReporterConfig &&config)
      : StreamingReporterBase(CATCH_MOVE(config)), m_tablePrinter(Detail::make_unique<TablePrinter>(m_stream, [&config]() -> std::vector<ColumnInfo> {
...
namespace Catch {
  ...
  void defaultListTags(std::ostream &out,
    ...
    if (!tags.empty()) {
      const auto maxTagCount = std::max_element(tags.begin(),
                                                tags.end(),
