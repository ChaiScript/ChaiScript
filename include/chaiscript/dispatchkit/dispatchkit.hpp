...
namespace chaiscript {
  ...
  namespace detail {
    ...
    public:
      ...
      ~Dispatch_Engine() {
        // Join all tracked async threads before m_state is destroyed,
        // to prevent heap-use-after-free when async threads access type maps
        // during error formatting (issue #636).
        join_async_threads();
      }
      ...
      static void pop_stack(Stack_Holder &t_holder) { t_holder.stacks.pop_back(); }
...
#pragma warning(pop)
      ...
      const StackData &get_stack_data() const noexcept { return m_stack_holder->stacks.back(); }

      static StackData &get_stack_data(Stack_Holder &t_holder) noexcept { return t_holder.stacks.back(); }
